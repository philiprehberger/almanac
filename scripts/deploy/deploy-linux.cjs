/**
 * Almanac - Release-Based Deployment Script (Linux/WSL)
 *
...
const CONFIG = {
    ...
    paths: {
        ...
        basePath: (process.env.SERVER_BASE_PATH || process.env.SERVER_DEST_PATH || '/var/www/almanac').trim(),
...
async function deploy(options = {}) {
    ...
    try {
        ...
        await execSSH(ssh, 'sudo supervisorctl restart almanac-horizon', { ignoreError: true });
        log('✅', 'Queue workers restarted');

        log('♻️', ' Restarting FastAPI embed worker...');
        await execSSH(ssh, 'sudo supervisorctl restart almanac-embed:*', { ignoreError: true });
        log('✅', 'Embed worker restarted');
        ...
        if (!options.skipVerify) {
            ...
            let healthOk = false;
            const healthUrl = `https://${process.env.MARKETING_DOMAIN || 'api.almanac.philiprehberger.com'}/v1/healthz`;
        ...
        log('🌐', 'Site: https://api.almanac.philiprehberger.com');
...
if (args.includes('--help') || args.includes('-h')) {
    console.log(`
Almanac - Release-Based Deployment (Linux/WSL)
